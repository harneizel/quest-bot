...
async def dont_understand(mess): # функция ответа на непонятные сообщения
    await mess.reply('Не понимаю вас, пожалуйста используйте кнопки')
...
async def cmd_start(message: Message):
    ...
    if message.from_user.id == config.ADMIN_ID:
        ...
        await message.answer('Добро пожаловать. Для управления используйте кнопки ниже', reply_markup=kb.main)
...
# захват номера оффалайн маршута
...
async def check_locate(callback: CallbackQuery):
 ...
 # сделать обработку локации по инлайн кнопке (или не сделать)
...
async def edited_message_handler(edited_message: Message):
    ...
    if flag1 == 1:
        ...
        dist = int(geodesic((a[0], a[1]), cords).meters)
        if dist>15:                         #если юзер не дошёл до точки
        ...
        elif dist<=15 and a[2]==a[3]:       # если юзер закончил маршрут
